  ...
  const handleGetOTP = async () => {
    ...
    if (!isValidPhoneNumber()) {
      showToast({
        type: 'error',
        title: 'Error',
        message: `Please enter a valid ${selectedCountryCode.country} mobile number (${selectedCountryCode.minLength} digits)`,
      });
      return;
    }
    ...
    try {
      ...
      if (!userId) {
        showToast({
          type: 'error',
          title: 'Error',
          message: 'Server se response nahi aaya. Please try again.',
        });
        setIsLoading(false);
        return;
      }
